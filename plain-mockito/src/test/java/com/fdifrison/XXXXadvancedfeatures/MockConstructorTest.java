...
public class MockConstructorTest {
  ...
  void mockConstructorCall() {
    ...
    // TODO we can instruct mockito to mock all the constructor calls of a class in a
    // try-with-resources block;
    //  calling a method of the object from outside the scope will invoke the concrete
    // implementation, we would then
    //  need to mock the whole object to be able to stub its methods.
    try (var mockedConstructor =
        mockConstruction(
            JpaUserRepository.class,
            // We can add additional stubbing
            (mock, context) -> {
              given(mock.findByUsername("fdifrison"))
                  .willReturn(new User().setUsername("fdifrison"));
