...
public class MockStaticMethodTest {
  ...
  // TODO To create mocks mockito use the MockMaker interface from the bytebuddy library and the
  // default behavior is
  //  subclassing.
  //  To be able to stub static method, mockito need a different mocking strategy called "inline"
  // (see in pom
  //  mockito-inline).
  ...
  void mockStaticMethodCalls() {
    ...
    try (var mockedLocalDateTime = mockStatic(LocalDateTime.class)) {
      // TODO the static mock implementation is thread local, therefore, outside the try-with
      // resources the
      //  LocalDateTime
